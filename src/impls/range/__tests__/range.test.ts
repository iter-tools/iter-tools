...
describe('range', () => {
  ...
  describe('when options are invalid', () => {
    it('throws', () => {
      expect(() => range({ start: 'foo' as any })).toThrowErrorMatchingSnapshot();
      expect(() => range({ end: null as any })).toThrowErrorMatchingSnapshot();
      expect(() => range({ step: Infinity as any })).toThrowErrorMatchingSnapshot();
    });
  });
});
