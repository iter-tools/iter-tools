...
describe('asyncSlice', () => {
  ...
  describe('with options argument', () => {
    it('can slice to the end', async () => {
      expect(await asyncToArray(asyncSlice({ start: 8 }, list))).toEqual([8, 9]);
    });
    ...
    it('returns simple slice with start/end/step', async () => {
      expect(await asyncToArray(asyncSlice({ start: 1, end: 6, step: 2 }, list))).toEqual([
        1,
        3,
        5,
      ]);
    ...
    it('returns curried slice', async () => {
      expect(await asyncToArray(asyncSlice({ start: 0, end: 2 })(list))).toEqual([0, 1]);
    });

    it('returns slice with negative end', async () => {
      expect(await asyncToArray(asyncSlice({ start: 4, end: -2 }, list))).toEqual(
        list.slice(4, -2),
      );
    });
    ...
    it('returns slice with negative end and step', async () => {
      expect(await asyncToArray(asyncSlice({ start: 1, end: -1, step: 3 }, list))).toEqual([
        1,
        4,
        7,
      ]);
    ...
    it('returns a smaller slice when start is negative and larger than source', async () => {
      expect(await asyncToArray(asyncSlice({ start: -3, end: 1, step: 1 }, [1, 2]))).toEqual([1]);
    });
  });
