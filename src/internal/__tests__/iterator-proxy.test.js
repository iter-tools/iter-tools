...
describe('IteratorProxy', () => {
  ...
  it('returns done after its source is done', () => {
    ...
    expect(proxy.next()).toEqual({ value: 0, done: false });
    expect(proxy.next()).toEqual({ value: 1, done: false });
    expect(proxy.next()).toEqual({ value: 2, done: false });
    expect(proxy.next()).toEqual({ value: undefined, done: true });
    expect(proxy.next()).toEqual({ value: undefined, done: true });
  });
  ...
  it('returns the value passed to return', () => {
    ...
    const proxy = new IteratorProxy(source);
    expect(proxy.next()).toEqual({ value: 0, done: false });
    expect(proxy.return()).toEqual({ value: undefined, done: true });
    expect(proxy.return(-1)).toEqual({ value: -1, done: true });
    expect(proxy.return(-2)).toEqual({ value: -2, done: true });
    expect(proxy.return()).toEqual({ value: undefined, done: true });
    expect(proxy.next()).toEqual({ value: undefined, done: true });
  });
  ...
  it('allows throw to be caught', () => {
    ...
    const proxy = new IteratorProxy(source);
    expect(proxy.next()).toEqual({ value: 0, done: false });
    expect(proxy.throw()).toEqual({ value: 1, done: false });
    expect(proxy.next()).toEqual({ value: 2, done: false });
    expect(proxy.throw()).toEqual({ value: undefined, done: true });
    expect(proxy.next()).toEqual({ value: undefined, done: true });
  });
});
