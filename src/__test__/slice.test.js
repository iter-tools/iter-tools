...
const { slice, asyncSlice, asyncIterToArray } = require('iter-tools')
...
describe('asyncSlice', function () {
  ...
  it('return simple slice', async function () {
    expect(await asyncIterToArray(asyncSlice(2, list))).toEqual([0, 1])
  })

  it('return simple slice with start/end', async function () {
    expect(await asyncIterToArray(asyncSlice({ start: 1, end: 4 }, list))).toEqual([1, 2, 3])
  })

  it('return simple slice with start/end/step', async function () {
    expect(await asyncIterToArray(asyncSlice({ start: 1, end: 6, step: 2 }, list))).toEqual([1, 3, 5])
  })
